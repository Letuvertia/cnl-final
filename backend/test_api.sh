...
curl -s -X POST -H "Content-Type: application/json" -d '{"username":"newuser1","password":"password123","email":"test@email.com"}' "$BASE_URL/register"
echo -e "\n"

# Test API 2: Authenticate a user (POST /login)
echo "Testing POST @api.route('/login', methods=['POST'])"
curl -s -X POST -H "Content-Type: application/json" -d '{"username":"newuser","password":"password123","latititude":"125.43","longitude":25.6}' "$BASE_URL/login"
echo -e "\n"
...
curl -s -X POST -H "Content-Type: application/json" -d '{"userid":"1","latitude":"125.34","longitude":"25.4"}' "$BASE_URL/location"
echo -e "\n"
