...
class MySQLConnector:
    def __init__(self, host="self.db", database="locasync", user="admin", password="admin") -> None:
    ...
    def _drop_table(self, table) -> None:
        query = f"DROP TABLE IF EXISTS {table}"
        self._execute_query(query, f"Drop table '{table}'")
    ...
    def clear_database(self) -> None:
        """
        Clear database. Start with empty tables.
        """
        self._drop_table("user")
        self._drop_table("messsage")
        self._drop_table("likes")
        self._create_user_table()
        self._create_message_table()
        self._create_likes_table()
    ...
    def update_likes(self, userid, msg_id) -> None:
        ...
        self._execute_query(query, f"Update message {msg_id} likes")
...
class Test:
    ...
    def run_unit_test(self):
        ...
        self.db.add_msg(msg_userid=test_msg['msg_userid'],
                        ...
                        msg_location_longitude=test_msg['msg_location_latitude'])
        print("Latest message: ", self.db.get_user_latest_msg(userid=1))
        print("User 1's message: ", self.db.get_user_all_msg(userid=1))
        print("=====Table 'message'=====")
        self.db._print_table("message")

        # location
        self.db.update_location(userid=1,
            latitude=test_msg['msg_location_latitude'], longitude=test_msg['msg_location_latitude'])
